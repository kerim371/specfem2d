...
\href{https://github.com/SPECFEM/specfem3d/tree/master/doc/USER_MANUAL}{SPECFEM3D Cartesian user manual}.\newline
...
\begin{itemize}
\item \texttt{ascii}: for reading in a single ASCII model file\newline
  (\texttt{DATA/proc***\_rho\_vp\_vs.dat})

\item \texttt{binary} or \texttt{gll}: for reading in binary model files\newline
  (\texttt{DATA/proc***\_rho.bin}, \texttt{DATA/proc***\_vp.bin}, \texttt{DATA/proc***\_vs.bin})

In case attenuation is turned on for the simulation, it will also read in attenuation files\newline
  (\texttt{DATA/proc***\_Qmu.bin}, \texttt{DATA/proc***\_Qkappa.bin})

\item \texttt{binary\_voigt}: for reading in an anisotropic model specified by binary files\newline
  (\texttt{DATA/proc***\_rho.bin}, \texttt{DATA/proc***\_c11.bin}, \texttt{DATA/proc***\_c13.bin}, \texttt{DATA/proc***\_c15.bin},\newline
   \texttt{DATA/proc***\_c33.bin}, \texttt{DATA/proc***\_c35.bin}, \texttt{DATA/proc***\_c55.bin})
...
\item \texttt{legacy}: for reading in a single ASCII model file with a legacy format of very old SPECFEM2D versions\newline
  (\texttt{DATA/proc***\_model\_velocity.dat\_input})
...
\item[{\texttt{read\_external\_mesh}}]
 If you are using an external mesher (like Gmsh, CUBIT/Trelis or GiD), you should set this parameter to \texttt{.true.}, with the following files defined for the mesher:
  \begin{description}
     ...
     \item[{\texttt{nummaterial\_velocity\_file}}] specifies the material properties for each material of the \texttt{nbmodels} defined in the materials file. The material specification follows a format slightly different than the \texttt{nbmodels} section in the \texttt{Par\_file}, but identical to the SPECFEM3D Cartesian version where you can find more informations about it. Also, examples of such \texttt{nummaterial\_velocity\_file} files can be found in the \texttt{EXAMPLES/meshing/} folder and the format is described in more detail therein in the header of the files.
...
and the mesh modules. An example is given in directory \texttt{EXAMPLES/applications/meshing/Gmsh\_example*}
...
%------------------------------------------------------------------------------------------------%
Cubit (or Trelis)%
\footnote{available at \url{https://coreform.com}}
is a 2D/3D finite element grid generator distributed which can be
used for the generation of quadrangle and hexahedral meshes. Cubit has a convenient
...
\url{https://coreform.com/coreform-cubit-tutorials/}
...
\texttt{EXAMPLES/reproducible\_study/Bottero2016\_axisymmetry\_example}
...
\texttt{EXAMPLES/applications/meshing/Gmsh\_example\_CPML\_MPI}.
...
To display mesh files, you can set the parameter \texttt{SAVE\_MESH\_FILES} in \texttt{DATA/Par\_file} to \texttt{.true.}
to visualize related files. This will save VTK files, as well as ASCII and/or Gnuplot files,
related to your simulation in folder \texttt{OUTPUT\_FILES}.

For example, to examine (using Gnuplot) how the mesh samples the wave field, type
%
\begin{verbatim}
