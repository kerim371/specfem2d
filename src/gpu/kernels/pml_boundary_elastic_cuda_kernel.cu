...
__global__ void pml_boundary_elastic_cuda_kernel(realw_p displ_elastic,
  ...
  if (id < pml_nglob_abs_elastic) {

    int iglob = d_pml_abs_points_elastic[id] - 1;

    // sets fields to zero at absorbing boundary
    for (int idim = 0; idim < NDIM; idim++) {
      displ_elastic[iglob * NDIM + idim] = 0.f;
      veloc_elastic[iglob * NDIM + idim] = 0.f;
      accel_elastic[iglob * NDIM + idim] = 0.f;
    }
  }
}
